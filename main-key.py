...
try:
	for journal in vaildWordsList:
		...
		for i in range(1,2000):
			...
			url = """https://pubmed.ncbi.nlm.nih.gov/?term=(%s[Title/Abstract]) AND (("%s/%s/%s"[Date - Publication] : "%s/%s/%s"[Date - Publication]))&sort=pubdate&filter=simsearch1.fha&page=%d"""%(journal,startTime[0],startTime[1],startTime[2],endTime[0],endTime[1],endTime[2],i)
			# print(url)
			driver.get(url)
